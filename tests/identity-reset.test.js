...
test('all public surfaces share the 4.0.1 Insta Toolbox identity', () => {
  assert.equal(packageMetadata.name, 'insta-toolbox');
  assert.equal(packageMetadata.version, '4.0.1');
  ...
  assert.match(userscriptMetadata, /@version\s+4\.0\.1\s*$/m);
  assert.match(serviceWorker, /const CACHE_NAME = 'insta-toolbox-v401'/);
});
...
test('Tampermonkey metadata uses the stable release channel', () => {
  ...
  assert.match(generatedUserscript, /@version\s+4\.0\.1\s*$/m);
