...
test('all public surfaces share the 4.0.0 Insta Toolbox identity', () => {
  ...
  assert.match(userscriptMetadata, /@version\s+4\.0\.0\s*$/m);
  assert.match(serviceWorker, /const CACHE_NAME = 'insta-toolbox-v400'/);
});
...
test('Tampermonkey metadata uses the stable release channel', () => {
  ...
  assert.match(generatedUserscript, /@version\s+4\.0\.0\s*$/m);
