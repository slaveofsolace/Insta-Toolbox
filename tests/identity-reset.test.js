...
test('all public surfaces share the 4.1.1 Insta Toolbox identity', () => {
  ...
  assert.match(userscriptMetadata, /@version\s+4\.1\.1\s*$/m);
  assert.match(serviceWorker, /const CACHE_NAME = 'insta-toolbox-v411'/);
});
...
test('Tampermonkey metadata uses the stable release channel', () => {
  ...
  assert.match(generatedUserscript, /@version\s+4\.1\.1\s*$/m);
