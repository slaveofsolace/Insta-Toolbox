...
test('all public surfaces share the 4.2.0 Insta Toolbox identity', () => {
  assert.equal(packageMetadata.name, 'insta-toolbox');
  assert.equal(packageMetadata.version, '4.2.0');
  ...
  assert.match(userscriptMetadata, /@version\s+4\.2\.0\s*$/m);
  assert.match(serviceWorker, /const CACHE_NAME = 'insta-toolbox-v420'/);
});
...
test('Tampermonkey metadata uses the stable release channel', () => {
  ...
  assert.match(generatedUserscript, /@version\s+4\.2\.0\s*$/m);
