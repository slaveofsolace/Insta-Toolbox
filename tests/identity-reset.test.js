...
test('all public surfaces share the 4.1.5 Insta Toolbox identity', () => {
  assert.equal(packageMetadata.name, 'insta-toolbox');
  assert.equal(packageMetadata.version, '4.1.5');
  ...
  assert.match(userscriptMetadata, /@version\s+4\.1\.5\s*$/m);
  assert.match(serviceWorker, /const CACHE_NAME = 'insta-toolbox-v415'/);
});
...
test('Tampermonkey metadata uses the stable release channel', () => {
  ...
  assert.match(generatedUserscript, /@version\s+4\.1\.5\s*$/m);
