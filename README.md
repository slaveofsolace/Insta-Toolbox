...
The stable Tampermonkey link always serves the latest published release. Version 4.1.5 files appear after the release checks finish.

| Surface | Release file | Use it when |
| --- | --- | --- |
| Tampermonkey | `insta-toolbox.user.js` | You want the simplest Instagram overlay install. |
| Chrome extension | `Insta-Toolbox-Extension-4.1.5.zip` | You prefer an unpacked browser extension. |
| Windows desktop | `Insta-Toolbox-Setup-4.1.5.exe` | You want one downloadable Windows installer. |
| macOS desktop | `Insta-Toolbox-4.1.5-universal.dmg` | You want the recommended drag-to-Applications package for Intel or Apple Silicon. |
| macOS portable | `Insta-Toolbox-4.1.5-universal.zip` | You prefer to extract the universal app directly. |
| Web/PWA | `insta-toolbox-web-4.1.5.zip` | You want to self-host the local-first workspace. |
...
The 4.1.5 development matrix covers the PWA, extension, userscript, layout controls, and packaged apps. The service worker uses cache generation `insta-toolbox-v415`. Automated fixtures do not prove current authenticated Instagram behavior; disposable-content acceptance is recorded separately.

See [Contributing](CONTRIBUTING.md), [Maintainer Guide](docs/MAINTAINER_GUIDE.md), [4.1.5 compatibility](docs/compatibility/4.1.5.md), and [4.1.5 acceptance](docs/acceptance/4.1.5.md).
