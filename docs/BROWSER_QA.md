...
| Fresh service-worker origin | Pass | The final source is reassembled and loaded from a fresh loopback origin using cache generation `insta-toolbox-v312`. |
...
3.1 repository suite contains 365 tests. Record the final pass against the exact
release commit in [acceptance/3.1.2.md](./acceptance/3.1.2.md).
...
- Apply Apple Developer ID signing and notarization when release credentials
  are available. Version 3.1.2 is universal and ad-hoc signed; CI validates the
