...
| Speed | No settings key in 3.1.12 | Standard single-thread pacing is internal. | Add Standard/Fast independent of scope. Keep Fast unavailable until its full pacing/verification contract passes. | Default Standard; never infer Fast from old minimum-delay values. |
...
2. **Cleanup defaults**: Speed, default message scope, own-reaction cleanup, summaries.
...
| Fast / reactions / workers / background preferences | Stored and capability-gated; Fast supported in the in-page candidate, other adapters disabled | Saved preferences cannot enable an unsupported adapter or restore run authority. Actual background behavior and reaction/inbox integration remain separate gates. |
| Per-run speed review | Implemented; Standard/Fast primary-button fixtures passed on both in-page surfaces | Speed is bound into the plan and confirmation, then revalidated before reservation. A changed speed dispatches nothing. Standard remains the default; live throughput remains unmeasured. |
