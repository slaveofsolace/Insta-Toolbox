...
| Own-reaction cleanup | Cleanup preferences `.removeOwnReactions` | Optional userscript pass over surviving messages after Unsend. Uses a separate adapter and verifies that only the signed-in account's reaction disappeared. | Retain as **Remove my reactions afterward**, default off. Other surfaces show the userscript requirement. | Add false; a saved value never authorizes an action by itself. |
...
| Execution mode | Cleanup preferences `.execution` | The userscript can open reviewed Ghost worker tabs in front or in the background. Loaded inactive tabs can continue; discarded, closed, signed-out, restricted, or sleeping tabs cannot. | Retain Foreground / Background on the userscript only, separate from cleanup scope. | Default Foreground. A saved preference never restores authority. |
| Worker count | Cleanup preferences `.workerCount` | The userscript opens a bounded pool of one to five exact reviewed conversation tabs. Tabs prepare in parallel; account mutations remain serialized. | Retain **Tabs to prepare** on the userscript. Other surfaces explain that managed tabs are unavailable. | Default one; migration never opens tabs or starts a job. |
...
| Own-reaction cleanup | Implemented in the userscript; fixture and rendered acceptance passed | A separate adapter proves current-user ownership, removes one exact reaction, verifies absence, preserves the message and other reactions, and reports unknown ownership as skipped. Authenticated disposable-reaction acceptance remains required. |
| Ghost workers and inactive-tab execution | Implemented in the userscript; fixture and rendered acceptance passed | One to five reviewed worker tabs, exclusive conversation ownership, durable non-authority checkpoints, stale-worker rejection, and serialized account mutations. Current authenticated lifecycle acceptance remains required. |
