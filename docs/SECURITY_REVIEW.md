...
The PWA service worker uses network-first same-origin GET handling, caches only successful same-origin responses, bypasses the HTTP cache for service-worker update checks, and removes earlier cache generations. Version 4.1.1 uses `insta-toolbox-v411`; persisted workspace and settings keys are unchanged.
...
The 4.1.1 development matrix contains 999 tests. The 45 overlay states and 11
...
tracked in [the 4.1.1 acceptance record](./acceptance/4.1.1.md). Fixtures do not
