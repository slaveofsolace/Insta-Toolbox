...
In `userscripts/src/toolbox-shell.js`, `checkAccountRelationships()` currently awaits `engine.fetchFollowerComparison(...)`. Replace that one call with `presenceInputs.captureComparison(...)`, retaining its existing progress, Stop, result-save and error handling. The wrapper returns the original result unchanged. It privately records a normalized snapshot in a WeakMap only when the same verified viewer is observed before and after the call, the requested and returned username match that viewer, and the response carries an observed numeric `subjectInstagramId` and current capture time.

After success, retain the original result reference only in the closure. Call `prepareProductionInputs({ capture: result, profile })` when preparing a Stay connected review. Never pass `state.capture` or a serialized result instead. Each new capture invalidates older receipts; `invalidate()` also revokes them. Stop/navigation/lifecycle handlers should invalidate the private adapter when abandoning that review context.
