...
This module is bundled for planning/review validation, but is not connected to a Start control. Do not pass its confirmation data to the existing batch executor until all of the following are implemented:
...
The shared profile engine now accepts optional runtime-only `assertAuthorized`, `assertContext`, and `signal` dependencies as a second argument. They are not accepted through message payloads. Dispatch checks run immediately before each native control, including after waiting for the Unfollow confirmation. Stop after dispatch permits read-only settlement; context loss or missing postcondition remains uncertain. This closes the per-control revocation prerequisite, not the numeric identity or shared-account ownership requirements.
