(() => {
  ...
  const styles = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .ia-header h1 { margin: 0; overflow-wrap: break-word; word-break: normal; font-size: 18px; line-height: 1.2; letter-spacing: -.015em; }
    ...
    .ia-move-handle { width: auto; min-width: 64px; grid-template-columns: 20px auto; gap: 5px; padding: 0 8px; cursor: grab; touch-action: none; }
    .ia-move-handle span { color: var(--ia-muted); font-size: 11px; font-weight: 700; }
    ...
    .ia-footer { display: flex; min-height: 44px; align-items: center; justify-content: space-between; gap: 10px; padding: 8px 54px 8px 14px; border-top: 1px solid var(--ia-line); background: color-mix(in srgb, var(--ia-surface-raised) var(--ia-panel-alpha-strong), transparent); color: var(--ia-muted); font-size: 12px; }
  ...
  function create({ document: targetDocument, openShadow = false }) {
    ...
    shadow.innerHTML = `
        ...
        <div class="ia-shell">
          ...
          <div class="ia-body">
            <header class="ia-header">
              <button class="ia-icon-button ia-move-handle" type="button" data-ia-role="move-handle" aria-label="Move sidecar; use arrow keys for precise movement" title="Drag to move · Arrow keys move">${icons.svg('move')}<span>Move</span></button>
            ...
            <div class="ia-scroll">
              ...
              <section class="ia-view" id="ia-view-capture" role="tabpanel" aria-labelledby="ia-tab-capture" tabindex="0" data-ia-view="capture" hidden>
                <div class="ia-state-row" data-ia-role="capture-state" data-tone="neutral"><span class="ia-state-dot"></span><div><strong data-ia-role="capture-state-title">Ready for a read-only check</strong><span data-ia-role="capture-state-detail">Confirm your Instagram username, then load Followers and Following.</span></div></div>
                <section class="ia-primary-action" aria-labelledby="ia-checker-account-title"><div class="ia-gate-summary"><span><strong id="ia-checker-account-title">Check an account</strong><span class="ia-note">Uses the Instagram session already open in this tab.</span></span><span class="ia-badge">read only</span></div><div class="ia-disclosure-body"><div class="ia-field"><label for="ia-checker-username">Instagram username</label><input class="ia-text-input" id="ia-checker-username" type="text" inputmode="text" autocomplete="username" autocapitalize="none" spellcheck="false" placeholder="your_username" data-ia-role="checker-username"></div><button class="ia-button" type="button" data-ia-action="check-account-relationships" data-ia-role="checker-run">Check Followers + Following</button><p class="ia-note">Loads paginated relationship data from Instagram, compares it on this device, and never follows, unfollows, sends, unsends, or opens account controls.</p></div></section>
                ...
                <details class="ia-disclosure"><summary>Advanced: list-dialog fallback and export</summary><div class="ia-disclosure-body"><p class="ia-note">If Instagram rejects the account check, open your own Following or Followers dialog and use the matching fallback scan. Starting a fallback scan clears authenticated results first so accounts from different profiles cannot mix.</p><ol class="ia-flow" aria-label="Follower checker fallback steps"><li class="ia-flow-step" data-ia-role="following-step"><span class="ia-flow-step-number">1</span><span class="ia-flow-step-copy"><strong>Scan open Following dialog</strong><span data-ia-role="following-step-detail">Open your Following list first</span></span><button class="ia-button ia-button--quiet" type="button" data-ia-action="scan-full-list" data-list-type="following">Scan</button></li><li class="ia-flow-step" data-ia-role="followers-step"><span class="ia-flow-step-number">2</span><span class="ia-flow-step-copy"><strong>Scan open Followers dialog</strong><span data-ia-role="followers-step-detail">Open your Followers list next</span></span><button class="ia-button ia-button--quiet" type="button" data-ia-action="scan-full-list" data-list-type="followers">Scan</button></li><li class="ia-flow-step" data-ia-role="compare-step"><span class="ia-flow-step-number">3</span><span class="ia-flow-step-copy"><strong>Compare</strong><span data-ia-role="compare-step-detail">Scan both lists first</span></span><span class="ia-badge" data-ia-role="compare-step-badge">waiting</span></li></ol><div class="ia-count"><strong data-ia-role="capture-count">0</strong><span data-ia-role="capture-detail">No draft yet</span></div><ul class="ia-list" data-ia-role="capture-list"></ul><div class="ia-field"><label for="ia-list-type-manual">List for manual capture</label><select class="ia-select" id="ia-list-type-manual" data-ia-role="list-type"><option value="following">Following</option><option value="followers">Followers</option></select></div><div class="ia-toolbar"><button class="ia-button ia-button--quiet" type="button" data-ia-action="capture-visible">Capture visible rows</button><a class="ia-link-button ia-link-button--quiet" data-ia-role="capture-download" aria-disabled="true">Download selected list</a><a class="ia-link-button ia-link-button--quiet" data-ia-role="comparison-json-download" aria-disabled="true">Download machine-readable JSON</a><button class="ia-button ia-button--quiet" type="button" data-ia-action="reset-capture">Clear checker</button></div></div></details>
              </section>
              <section class="ia-view" id="ia-view-queue" role="tabpanel" aria-labelledby="ia-tab-queue" tabindex="0" data-ia-view="queue" hidden>
                ...
                <section class="ia-primary-action" data-ia-role="bot-disclosure" aria-labelledby="ia-bot-composer-title"><div class="ia-gate-summary"><span><strong id="ia-bot-composer-title">Run composer</strong><span class="ia-note">Inspect exact profiles before any live action.</span></span><span class="ia-badge" data-ia-role="bot-badge">idle</span></div><div class="ia-disclosure-body">
              ...
              <section class="ia-view" id="ia-view-messages" role="tabpanel" aria-labelledby="ia-tab-messages" tabindex="0" data-ia-view="messages" hidden>
                ...
                <div class="ia-primary-action" data-ia-role="unsend-disclosure"><div class="ia-gate-summary"><span><strong>DM Unsend</strong></span><span class="ia-badge" data-ia-role="unsend-badge">ready to check</span></div><div class="ia-disclosure-body">
                  <p class="ia-gate-detail" data-ia-role="unsend-detail">Unsend DMs checks this conversation first when needed, then asks once before removing anything.</p>
                  <p class="ia-note" data-ia-role="unsend-eligible">Check this conversation to resolve the eligible count</p>
                  <div class="ia-toolbar"><button class="ia-button ia-button--danger" type="button" data-ia-action="mass-unsend">Unsend DMs</button></div><p class="ia-note">Unsending is permanent. The exact thread, scope, count, digest, and expiry are revalidated before the first message menu opens.</p>
                </div></div>
            ...
            <footer class="ia-footer" role="status" aria-live="polite" data-ia-role="status"><span class="ia-footer-message"><strong data-ia-role="status-lead">Ready.</strong> <span data-ia-role="status-text">Review an exact finite run before any change.</span></span><span class="ia-footer-lock">Local only</span></footer>
          </div>
        </div>
      ...
      <template data-ia-template="advanced-settings"><strong>Batch pacing and limits</strong><div class="ia-field"><label for="ia-limit-actions">Follow/unfollow per day</label><input class="ia-text-input" id="ia-limit-actions" type="number" min="1" max="400" data-ia-role="limit-actions"></div><div class="ia-field"><label for="ia-limit-dms">Unsends per day</label><input class="ia-text-input" id="ia-limit-dms" type="number" min="1" max="300" data-ia-role="limit-dms"></div><div class="ia-field"><label for="ia-limit-min-delay">Min delay (seconds)</label><input class="ia-text-input" id="ia-limit-min-delay" type="number" min="2" max="600" data-ia-role="limit-min-delay"></div><div class="ia-field"><label for="ia-limit-max-delay">Max delay (seconds)</label><input class="ia-text-input" id="ia-limit-max-delay" type="number" min="2" max="900" data-ia-role="limit-max-delay"></div><button class="ia-button ia-button--quiet" type="button" data-ia-action="save-limits">Save pacing</button></template>
      <template data-ia-template="first-run"><section class="ia-first-run" data-ia-role="first-run" aria-labelledby="ia-first-run-title"><h2 id="ia-first-run-title">Start with a read-only check</h2><p>Insta AIO adds three tools to Instagram without sending account data elsewhere.</p><ol><li><strong>Follower checker</strong><span>Scan Followers and Following, then compare usernames on this device.</span></li><li><strong>Follow / Unfollow</strong><span>Build and review an exact finite target list, then confirm it once.</span></li><li><strong>DM Unsend</strong><span>Check one conversation, then confirm the exact thread and eligible count once.</span></li></ol><div class="ia-toolbar"><button class="ia-button" type="button" data-ia-action="first-run-start">Open follower checker</button><button class="ia-button ia-button--quiet" type="button" data-ia-action="first-run-dismiss">Not now</button></div></section></template>
